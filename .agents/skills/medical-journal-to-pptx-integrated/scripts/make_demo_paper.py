#!/usr/bin/env python3
"""Generate a small, clearly fictional medical-imaging article for class practice."""
...
def synthetic_workflow_image() -> bytes:
    ...
    draw.text((48, 35), "SYNTHETIC IMAGING WORKFLOW — FICTIONAL DATA", fill="#102444", font=title_font)

    boxes = [
        ((48, 155, 378, 495), "Image acquisition", ["120 synthetic cases", "Three image readers", "No patient data"]),
        ((470, 155, 800, 495), "Structured review", ["Standardized checklist", "Reader calibration", "Fictional reference"]),
        ((892, 155, 1222, 495), "Teaching outcome", ["Sensitivity: 88%", "Specificity: 82%", "Illustrative only"]),
    ]
...
def add_header(page: fitz.Page, number: int) -> None:
    ...
    page.insert_text(
        (42, 35),
        "SYNTHETIC CLASSROOM JOURNAL | NOT A REAL STUDY",
        fontsize=12,
        color=(1, 1, 1),
        fontname="hebo",
    )
    page.insert_text((42, 810), f"Classroom demonstration / Page {number}", fontsize=9, color=GREY)
...
def draw_table(page: fitz.Page, top: float) -> float:
    ...
    rows = [
        ...
        ("Sensitivity", "88%", "76%", "+12 points"),
        ("Specificity", "82%", "79%", "+3 points"),
        ("Median review time", "4.2 min", "5.1 min", "-0.9 min"),
        ("Reader agreement", "0.74", "0.61", "+0.13"),
    ]
    row_height = 33
...
def create_demo_paper(destination: Path) -> Path:
    ...
    add_header(page, 1)
    y = 94
    y = add_heading(page, "Synthetic Evaluation of a Structured Imaging Workflow", y)
    y = add_paragraph(
        page,
        "Classroom Education Team | Synthetic Journal of Imaging Education | 2026. "
    ...
    y = add_paragraph(
        page,
        "Purpose: demonstrate how an imaging journal can be converted into a teaching presentation. "
        "Methods: 120 fictional examinations were reviewed with two imaginary workflows. "
    ...
    y = add_paragraph(
        page,
        "A reusable classroom example must avoid copyrighted articles and identifiable patient data. "
    ...
    y = add_paragraph(
        page,
        "The simulated cohort contains 120 invented cases. Three fictional readers apply a structured "
        "checklist. The reference standard, outcomes, and all confidence estimates are fabricated. "
        "No human participants, protected health information, or real imaging datasets are included.",
        y,
        height=72,
    )
    page.insert_text((46, y + 8), "Study design: synthetic classroom simulation", fontsize=10, color=BLUE, fontname="hebo")
    ...
    y = add_paragraph(
        page,
        "The diagram illustrates the imaginary transition from image acquisition to structured review "
    ...
    y = add_paragraph(
        page,
        "Figure 1. Synthetic image-review workflow with fictional sensitivity and specificity. "
        "This figure is generated for the classroom and contains no patient images.",
        y,
        height=52,
    )
    y = add_heading(page, "Imaging Interpretation", y)
    add_paragraph(
        page,
    ...
    document.set_metadata(
        {
            "title": "Synthetic Evaluation of a Structured Imaging Workflow",
            "author": "Classroom Education Team",
            "subject": "Fictional article for software testing; not clinical evidence",
            "keywords": "synthetic, classroom, fictional, no patient data",
        }
    )
...
def main() -> int:
    ...
    print(f"Created synthetic classroom paper: {output}")
    return 0
