...
def canonical_svg(
    ...
    *,
    ...
    pad_top: float,
    pad_bottom: float,
    expected_text: list[str] | tuple[str, ...] = (),
) -> tuple[bytes, list[float], float]:
    """Validate source bounds, then return a white-padded canonical SVG."""
    ...
    with pymupdf.open(pdf_path) as document:
        ...
        source_rect, _ = region(pdf_page, parsed_bbox, expected_text)
        effective = [float(value) for value in source_rect]
        with pymupdf.open() as padded:
            padded_page = padded.new_page(
                width=source_rect.width + 2 * float(pad_x),
                height=source_rect.height + float(pad_top) + float(pad_bottom),
            )
            target = pymupdf.Rect(
                float(pad_x), float(pad_top),
                float(pad_x) + source_rect.width,
                float(pad_top) + source_rect.height,
            )
            padded_page.show_pdf_page(target, document, page - 1, clip=source_rect)
            svg_text = padded_page.get_svg_image(matrix=pymupdf.Matrix(1, 1))
    ...
    aspect = (
        (source_rect.width + 2 * float(pad_x))
        / (source_rect.height + float(pad_top) + float(pad_bottom))
    )
...
def replay_vector_table(
    ...
    *,
    ...
    if not isinstance(expected_text, list) or not all(
        ...
        expected_text = None
    if sidecar.get("padding_mode") != "blank_canvas":
        failures.append(f"{prefix} must add padding on a blank canvas.")
    ...
    if (
        failures or requested is None or effective is None or padding is None
        or expected_text is None or not isinstance(page, int)
    ):
        return failures
    try:
