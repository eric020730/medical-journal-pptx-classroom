...
def validate_specification(
    ...
    for index, slide in enumerate(slides, start=1):
        ...
        if not image.is_file():
            ...
            continue
        if image.suffix.lower() in {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".webp"}:
            raster_figures += 1
    ...
    if mode == "full":
        ...
        if counts.get("part", 0) == 0 and counts.get("section", 0) == 0:
    ...
    elif audit_images and raster_figures:
        failures.append(
            "Raster figure(s) require a readable extraction manifest; original-PDF "
            "image polarity and complete intermediate provenance cannot be verified."
        )
            ...
            if slide_type == "figure":
                ...
                if not expected_labels and isinstance(image, str) and spec_path is not None:
                    ...
                    if sidecar.get("native_labels"):
                        expected_labels = sidecar.get("labels") or []
                if isinstance(expected_labels, list):
                    visible_labels = {
                        text.strip().upper() for text in visible_texts(slide)
                    }
                    missing_labels = [
                        str(label).strip().upper()
                        for label in expected_labels
                        if str(label).strip().upper() not in visible_labels
                    ]
                    if missing_labels:
                        failures.append(
                            f"Figure slide {index} is missing visible native panel "
