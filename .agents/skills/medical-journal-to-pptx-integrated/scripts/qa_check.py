...
def _check_source_inventory_panels(
    presentation: Any,
    specification: dict[str, Any],
    spec_path: Path,
    expected_logo_hash: str | None,
) -> list[str]:
    """Bind 4.6 source-crop inventories to exact native labels in the PPTX."""
    failures: list[str] = []
    plan_value = specification.get("meta", {}).get("panel_crop_plan")
    ...
    for index, (slide, slide_spec) in enumerate(
        zip(presentation.slides, spec_slides), start=1
    ):
        if not isinstance(slide_spec, dict) or slide_spec.get("type") != "figure":
            continue
        key = ""
        source_id = slide_spec.get("source_asset_id")
        if isinstance(source_id, str) and re.fullmatch(r"figure:\d+", source_id, re.I):
            key = "Figure_" + source_id.split(":", 1)[1]
        if not key:
            match = FIGURE_RE.search(str(slide_spec.get("caption", "")))
            key = "Figure_" + match.group(1) if match else ""
        ...
        if not pictures:
            failures.append(f"Slide {index}: source-inventory figure {key} has no picture.")
            continue
        picture = max(pictures, key=lambda shape: int(shape.width) * int(shape.height))
        for label, geometry_item in zip(labels, geometry):
            matches = [
                shape for shape in slide.shapes
                if getattr(shape, "has_text_frame", False)
                and shape.text_frame.text.strip() == str(label)
            ]
            if len(matches) != 1:
                failures.append(
                    f"Slide {index}: expected exactly one native {label}; found {len(matches)}."
                )
                continue
            shape = matches[0]
            runs = [
            ...
            try:
                ...
                expected_center = (
                    int(picture.top) + float(geometry_item["fy_center"]) * int(picture.height)
                )
                exact_position = (
                    abs(int(shape.left + shape.width) - expected_right) <= 0.02 * 914400
                    and abs(int(shape.top + shape.height / 2) - expected_center) <= 0.02 * 914400
                )
            except (AttributeError, KeyError, TypeError, ValueError):
                exact_style = exact_position = False
            if not exact_style or not exact_position:
                failures.append(
                    f"Slide {index}: native {label} has the wrong style or position."
                )
    for key in sorted(inventory.keys() - seen):
...
def validate_presentation(
    ...
    *,
    ...
    if specification_data is not None and spec_path is not None:
        try:
            failures.extend(
                _check_source_inventory_panels(
                    presentation, specification_data, spec_path, expected_logo
                )
            )
        except (KeyError, OSError, TypeError, ValueError, json.JSONDecodeError) as error:
            failures.append(f"Native panel inventory QA could not complete: {error}")
