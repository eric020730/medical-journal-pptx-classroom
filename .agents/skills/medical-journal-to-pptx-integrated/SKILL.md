...
2. Run `scripts/run.py doctor --strict`; LibreOffice and Poppler are mandatory
   for PDF export and every-slide preview. Then run
...
4. Process assets with the bundled helpers; each paper Figure becomes exactly
   ...
   multi-seam evidence, replay, label, and bounded-cleanup contracts. For tables
   or reviewed PDF-coordinate panels, write a source-hash-bound crop plan with
   title/header/last-row/footnote anchors and run `scripts/run.py run
   source_crops -- <plan.json> --out <new-directory>`; never expand source
   bounds for padding or reuse another paper's coordinates.
   ...
   Classify by content: clinical images use `--asset-type clinical-image` (or
   the compatible `--medical-image`) with active peripheral white-edge cleanup
   and a 0 px outer canvas; figures/flowcharts use exactly 16 px; raster tables
   use 8–24 px (default 16).
...
7. Run `scripts/run.py qa <output.pptx> --spec <spec.json> --mode full
   ...
   locally rewritten manifest are never sufficient. Run `scripts/run.py render
...
Never upload or publish source papers, patient data, generated decks,
credentials, or run artifacts without explicit user authorization.
