...
## Portable Classroom Runtime And Output

This is the preserved full upstream v0.2.38 workflow, adapted for the portable
classroom repository. Resolve the repository root from `.classroom-project.json`.
Use its `.venv/bin/python` on macOS/Linux or `.venv/Scripts/python.exe` on
Windows. Prefer the root `journal` / `journal.cmd` launcher so LibreOffice and
Poppler are discovered even when they are not on `PATH`.

Save every completed PowerPoint and optional PDF directly in the repository's
`outputs/` directory. Keep run-specific intermediate assets, manifests, specs,
and previews in `.skill-work/<run-id>/`. Do not use a personal Desktop path,
an absolute username-based Python path, or a cloud source directory.
