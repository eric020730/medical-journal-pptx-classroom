...
5. Write a fresh `deck_spec.json`. Visible slide content is English. Every slide receives substantive, scan-friendly Traditional Chinese notes with accurate English medical terminology and article-specific explanations. Use the bundled logo and visual style unless the user provides an authorized replacement.
   Medical image panels (CT, MRI, radiographs, ultrasound, nuclear medicine and other diagnostic images) require a separate peripheral white-edge cleanup after source validation: compose with `--medical-image`, not `--no-trim`. Keep the full source crop for comparison. Inspect cleaned edges for retained anatomy, arrows, measurements and scale bars. Never apply medical-image cleanup to tables, plots or flowcharts. Do not deliver visible residual white rims without repairing or explaining the source-specific limitation.
   For photographic panels, set `meta.panel_crop_plan` to the reviewed plan with `export_image_panels: true`. Use the banded compositor's geometry and one post-build native-label pass; omit builder `panel_labels`. A failed image-only crop must be repaired, never replaced by a white review crop. Final QA must check the source inventory, native label count, 18pt size, #8FA8C8 color and position.
