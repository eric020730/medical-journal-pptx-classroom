#!/usr/bin/env bash
# Agent-friendly full-quality setup. No sudo, Homebrew, profile edits, or global skills.
set -Eeuo pipefail
...
export PIXI_HOME="$ROOT/.bootstrap/pixi-home"
export PIXI_CACHE_DIR="$ROOT/.bootstrap/pixi-cache"
...
if [[ "${1:-}" == '--check-only' ]]; then
  [[ $# == 1 ]] || exit 2
  "$PY" "$QUALITY" check --json
  exec "$PY" "$ROOT/tools/codex_setup.py" --check
fi
[[ $# == 0 ]] || { echo 'Usage: bash setup-codex.sh [--check-only]'; exit 2; }

mkdir -p .bootstrap .skill-work
mkdir .bootstrap/setup.lock 2>/dev/null || {
  echo 'SETUP_BLOCKED: another setup may be active; do not delete its lock blindly'; exit 1;
}
TMP=''
cleanup() {
  [[ -z "$TMP" ]] || rm -rf -- "$TMP"
  rmdir .bootstrap/setup.lock 2>/dev/null || true
}
trap cleanup EXIT
[[ ! -L .skill-work/codex-setup.json ]] || {
  echo 'SETUP_BLOCKED: receipt is a symlink'; exit 1;
}
...
if [[ -x "$PY" ]] && "$PY" tools/codex_setup.py --dependencies-ready; then
  echo 'Reusing this project Python environment.'
else
  ...
  [[ "$ACTUAL" == "$HASH" ]] || {
    echo 'SETUP_BLOCKED: uv checksum mismatch; nothing executed'; exit 1;
  }
  tar -xzf "$ARCHIVE" -C "$TMP"
  UV="$TMP/uv-$TARGET/uv"
  [[ -x "$UV" ]] || {
    echo 'SETUP_BLOCKED: verified uv archive layout unexpected'; exit 1;
  }
  if [[ ! -e .venv ]]; then
...
echo 'Preparing fixed LibreOffice and Poppler rendering tools inside this project.'
"$PY" "$QUALITY" install --json
# This gate builds a synthetic PPTX, renders it to PDF and creates slide previews.
