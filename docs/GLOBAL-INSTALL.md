...
`medical-journal-to-pptx-integrated v4.6.0` 是真正獨立的全域 Codex
...
2. 下載 `medical-journal-to-pptx-integrated-v4.6.0.zip` 及同名 `.sha256`。
...
```bash
shasum -a 256 -c medical-journal-to-pptx-integrated-v4.6.0.zip.sha256
unzip medical-journal-to-pptx-integrated-v4.6.0.zip
cd medical-journal-to-pptx-integrated-v4.6.0
...
```powershell
$archive = "medical-journal-to-pptx-integrated-v4.6.0.zip"
$expected = ((Get-Content "$archive.sha256") -split "\s+")[0]
$actual = (Get-FileHash $archive -Algorithm SHA256).Hash.ToLowerInvariant()
if ($actual -ne $expected) { throw "SHA-256 mismatch" }
Expand-Archive $archive -DestinationPath .
Set-Location .\medical-journal-to-pptx-integrated-v4.6.0
