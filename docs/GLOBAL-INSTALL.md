...
`medical-journal-to-pptx-integrated v4.7.0-rc.1` 是真正獨立的全域 Codex
...
2. 下載 `medical-journal-to-pptx-integrated-v4.7.0-rc.1.zip` 及同名 `.sha256`。
...
```bash
shasum -a 256 -c medical-journal-to-pptx-integrated-v4.7.0-rc.1.zip.sha256
unzip medical-journal-to-pptx-integrated-v4.7.0-rc.1.zip
cd medical-journal-to-pptx-integrated-v4.7.0-rc.1
...
```powershell
$archive = "medical-journal-to-pptx-integrated-v4.7.0-rc.1.zip"
$expected = ((Get-Content "$archive.sha256") -split "\s+")[0]
$actual = (Get-FileHash $archive -Algorithm SHA256).Hash.ToLowerInvariant()
if ($actual -ne $expected) { throw "SHA-256 mismatch" }
Expand-Archive $archive -DestinationPath .
Set-Location .\medical-journal-to-pptx-integrated-v4.7.0-rc.1
...
或任何專案的 `.venv`。LibreOffice 和 Poppler 是最終完整品質門檻的一部分；
缺少時仍可保留中間 PPTX，但不能宣稱已完成最終簡報。
...
native panel labels，以及 PDF 灰階反相防護；完成建檔前／後 QA、PDF 匯出
與全部 slide previews 檢查後，將最終 PPTX 和 PDF 儲存到我指定的輸出資料夾。
```
