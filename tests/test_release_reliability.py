...
class ReleaseVersionTests(unittest.TestCase):
    def test_drift_blocks_check_and_sync_repairs_only_current_metadata(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for name in (".classroom-project.json", *release_version.DOCUMENTS,
            ...
            self.assertEqual(len(release_version.synchronize(root)), 3)
            ...
            self.assertEqual(release_version.metadata(root)["archive"], "medical-journal-to-pptx-integrated-v9.8.7.zip")
